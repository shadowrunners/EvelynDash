        ...
        <div className="mt-5 min-h-screen font-sans mb-8">
            ...
            <QueryStatusHelper status={status} error={error?.message} refetch={refetch}>
                <div className='sm:flex-col flex-row grid grid-cols-1 sm:grid-cols-3 gap-3 mt-5 mx-8 sm:mx-16 md:mx-16'>
                    {data?.map((guild, i) => (
                        <CardSpotlight onClick={() => router.push(guild.botPresent ? `/guilds/${guild.id}` : `https://discord.com/oauth2/authorize?client_id=${process.env.NEXT_PUBLIC_CLIENT_ID}&permissions=8&scope=bot%20applications.commands&guild_id=${guild.id}`)} className='text-white hover:cursor-pointer rounded-xl' key={`guild-${i}`}>
